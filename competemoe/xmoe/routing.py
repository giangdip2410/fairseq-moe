...
class Top1Gate(torch.nn.Module):
    ...
    def forward(self, input, mask=None):  # type: ignore
        ...
        return top1gating(
            ...
            gate_obj=self,gate_inp=self.gate_inp
        )
...
def top2gating(
    ...
    batch_prioritized_routing=False,gate_inp=None
    ...
    if use_fp32:
        return l_aux, combine_weights.to(orig_dtype), dispatch_mask, metadata, gates
    else:
...
class Top2Gate(torch.nn.Module):
    ...
    def forward(self, input, mask=None, gate_inp=None):  # type: ignore
        if self.use_xmoe:
            input = self.wg_reduction(input)
            with torch.no_grad():
                wg_norm = self.wg.norm(p=2.0, dim=1, keepdim=True)
