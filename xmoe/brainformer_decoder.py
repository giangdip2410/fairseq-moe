...
from .xmoe.moe_layer import MOELayer
from .xmoe.routing import Top1Gate, Top2Gate
try:
...
class BrainformerDecoderLayer(nn.Module):
    def __init__(
        ...
        self.ffn1 = self.build_ffn(
            self.embed_dim,
            self.args,
        )
        self.ffn2 = self.build_ffn(
            self.embed_dim,
            self.args,
        )
        self.ffn3 = self.build_ffn(
            self.embed_dim,
            self.args,
        )
        # else:
        ...
        experts1 = make_experts(args, self.embed_dim, self.ffn_dim)
        self.moe_layer1 = MOELayer(gate1, experts1, args)
        #build MOE2
        if args.moe_top1_expert:
            gate2 = Top1Gate(
                self.embed_dim,
        ...
        experts2 = make_experts(args, self.embed_dim, self.ffn_dim)
        self.moe_layer2 = MOELayer(gate2, experts2, args)

        #normalization
        self.final_layer_norm = LayerNorm(self.embed_dim, eps=args.layernorm_eps)
        self.final_layer_norm1 = LayerNorm(self.embed_dim, eps=args.layernorm_eps)
        self.final_layer_norm2 = LayerNorm(self.embed_dim, eps=args.layernorm_eps)
        self.final_layer_norm3 = LayerNorm(self.embed_dim, eps=args.layernorm_eps)
        self.final_layer_norm4 = LayerNorm(self.embed_dim, eps=args.layernorm_eps)
        self.final_layer_norm5 = LayerNorm(self.embed_dim, eps=args.layernorm_eps)
        self.final_layer_norm6 = LayerNorm(self.embed_dim, eps=args.layernorm_eps)
...
class BrainformerDecoder(nn.Module):
    def __init__(
        self,
        args,
        embed_tokens=None,
