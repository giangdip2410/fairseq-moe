...
class BrainformerEncoderLayer(nn.Module):
    """Encoder layer block.
    ...
    *args.encoder_normalize_before* to ``True``.
    ...
    def __init__(self, args, is_moe_layer=True):
        super().__init__()
        self.args = args
        ...
        #build MOE1
        gate1=self.build_gate(args)
        experts1 = self.make_experts(args, self.embed_dim, ffn_dim, self.dropout_module)
        self.moe_layer1 = self.build_moe_layer(gate1, experts1, args)
        #build MOE2
        gate2=self.build_gate(args)
        experts2 = self.make_experts(args, self.embed_dim, ffn_dim, self.dropout_module)
        self.moe_layer2 = self.build_moe_layer(gate2, experts2, args)
        #build norm layer
        self.final_layer_norm = LayerNorm(self.embed_dim)
        self.final_layer_norm1 = LayerNorm(self.embed_dim)
        self.final_layer_norm2 = LayerNorm(self.embed_dim)
        self.final_layer_norm3 = LayerNorm(self.embed_dim)
        self.final_layer_norm4 = LayerNorm(self.embed_dim)
        self.final_layer_norm5 = LayerNorm(self.embed_dim)
        self.final_layer_norm6 = LayerNorm(self.embed_dim)
    ...
    def forward(self, x, encoder_padding_mask: Optional[Tensor], attn_mask: Optional[Tensor] = None):
        ...
        residual = x
        if self.normalize_before:
            x = self.final_layer_norm2(x)
        # x - seq_len, batch_size, model_dim
        x = x.transpose(0, 1) # batch_size, seq_len, model_dim
        if getattr(self.args, "use_moe_pad_mask", False):
            x, _l_aux = self.moe_layer1(x, input_padding_mask=encoder_padding_mask)
        else:
            x, _l_aux = self.moe_layer1(x)
        l_aux += _l_aux
        x = x.transpose(0, 1) # seq_len, batch_size, model_dim
        x = self.residual_connection(x, residual)
        if not self.normalize_before:
        ...
        residual = x
        if self.normalize_before:
            x = self.final_layer_norm3(x)
        # x - seq_len, batch_size, model_dim
        x = x.transpose(0, 1) # batch_size, seq_len, model_dim
        if getattr(self.args, "use_moe_pad_mask", False):
            x, _l_aux = self.moe_layer2(x, input_padding_mask=encoder_padding_mask)
        else:
            x, _l_aux = self.moe_layer2(x)
        l_aux += _l_aux
        del _l_aux
        x = x.transpose(0, 1) # seq_len, batch_size, model_dim
        x = self.residual_connection(x, residual)
        ...
        residual = x
        if self.normalize_before:
            x = self.final_layer_norm4(x)
        
        x = self.activation_fn(self.fc3(3))
        x = self.activation_dropout_module(x)
        x = self.fc4(x)
        x = self.dropout_module(x)
        x = self.residual_connection(x, residual)
        ...
        residual = x
        if self.normalize_before:
            x = self.final_layer_norm5(x)
        
        x = self.activation_fn(self.fc5(3))
        x = self.activation_dropout_module(x)
        x = self.fc6(x)
        x = self.dropout_module(x)
        x = self.residual_connection(x, residual)
        ...
        residual = x
        if self.normalize_before:
            x = self.final_layer_norm6(x)
        
        x = self.activation_fn(self.fc7(3))
        x = self.activation_dropout_module(x)
        x = self.fc8(x)
        x = self.dropout_module(x)
        x = self.residual_connection(x, residual)
...
class BrainformerDecoderLayer(nn.Module):
    """Decoder layer block.
    ...
    *args.decoder_normalize_before* to ``True``.
    ...
    def forward(
        ...
        x = x.transpose(0, 1) # batch_size, seq_len, model_dim
        if getattr(self.args, "use_moe_pad_mask", False):
            x, _l_aux = self.moe_layer1(x, input_padding_mask=self_attn_padding_mask)
        else:
            x, _l_aux = self.moe_layer1(x)
        l_aux += _l_aux
        x = x.transpose(0, 1) # seq_len, batch_size, model_dim
        ...
        x = x.transpose(0, 1) # batch_size, seq_len, model_dim
        if getattr(self.args, "use_moe_pad_mask", False):
            x, _l_aux = self.moe_layer2(x, input_padding_mask=self_attn_padding_mask)
        else:
            x, _l_aux = self.moe_layer2(x)
        l_aux += _l_aux
        del _l_aux
